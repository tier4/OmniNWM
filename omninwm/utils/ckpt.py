...
from omninwm.utils.logger import log_message
...
def load_from_sharded_state_dict(
    ...
    """
    # Check if this is a HuggingFace sharded checkpoint
    hf_index_file = os.path.join(ckpt_path, f"{model_name}.safetensors.index.json")
    if os.path.exists(hf_index_file):
        log_message(f"Detected HF sharded checkpoint format at {ckpt_path}")
        load_from_hf_sharded_checkpoint(model, ckpt_path, model_name, strict)
    else:
        ckpt_io = GeneralCheckpointIO()
        ckpt_io.load_model(model, os.path.join(ckpt_path, model_name), strict=strict)


def load_from_hf_sharded_checkpoint(
    model: nn.Module, ckpt_path: str, model_name: str = "model", strict=False
):
    """
    Loads a model from HuggingFace sharded safetensors checkpoint.
    ...
    """
    index_file = os.path.join(ckpt_path, f"{model_name}.safetensors.index.json")
    ...
    for key, shard_name in weight_map.items():
        ...
        shard_files[shard_name].append(key)

    log_message(f"Loading {len(weight_map)} weights from {len(shard_files)} shards...")

    # Load all shards
    state_dict = {}
    for shard_name, keys in shard_files.items():
        shard_path = os.path.join(ckpt_path, shard_name)
        log_message(f"Loading shard: {shard_name} ({len(keys)} tensors)")
...
def load_checkpoint(
    ...
    if path.endswith(".safetensors"):
        ...
        #     if name in network_dict.keys():
        #         if ckpt[name].shape == network_dict[name].shape:
        #             network_dict[name].copy_(param)

        # missing, unexpected = model.load_state_dict(network_dict, strict=strict)
        # print_load_warning(missing, unexpected)
