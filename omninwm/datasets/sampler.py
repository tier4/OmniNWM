...
from omninwm.utils.misc import format_numel_str
...
class VariableVideoBatchSampler(DistributedSampler):
    def __init__(
        ...
        super().__init__(
            dataset=dataset, num_replicas=num_replicas, rank=rank, shuffle=shuffle, drop_last=drop_last
        )
    ...
    def group_by_bucket(self) -> dict:
        ...
        for i in valid_indices:
            # bucket_sample_dict[tuple(bucket_ids_np[i])].append(i)
            bucket_sample_dict[(bucket_ids_np[i][0],int(bucket_ids_np[i][1]),bucket_ids_np[i][2])].append(i)
