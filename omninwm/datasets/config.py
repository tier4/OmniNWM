...
class DatasetConfig:
    ...
    seg_class_map: Dict[int, int] = field(
        default_factory=lambda: {
            0: 0,
            1: 1,
            2: 2,
    ...
    seg_color_map: List[Tuple[int, int, int]] = field(
        default_factory=lambda: [
            (0, 0, 0),  # object
            (0, 207, 191),  # road
            (135, 206, 235),  # sky
    ...
    # T4 dataset specific attributes
    seg_root: Optional[str] = None  # Root directory for SAM3 segmentation
    depth_root: Optional[str] = None  # Root directory for Prior-Depth-Anything
    depth_scale: Optional[float] = (
        None  # Scale factor for depth (650 for Prior-Depth-Anything)
    )
    seg_png_format: bool = False  # Whether segmentation is stored as PNG
    depth_png_format: bool = False  # Whether depth is stored as PNG
    t4_camera_map: Optional[Dict[str, str]] = (
        None  # Map OmniNWM camera names to T4 camera names
    )
    # Optional per-camera binary masks for ray-map zeroing (soft mask in geometry space)
    # Dict key: camera name in view_order (e.g., CAM_FRONT), value: mask image path
